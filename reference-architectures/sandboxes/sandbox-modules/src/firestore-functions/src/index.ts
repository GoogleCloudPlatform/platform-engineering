...
exports.deploymentCreated = onDocumentCreated(
  ...
  async event => {
    ...
    console.log('[FirestoreTrigger] 📝 New deployment created:', deploymentId, {
      deploymentId,
    });
    ...
    if (data._updateSource === 'cloudrun') {
      console.log(
        '[FirestoreTrigger] ⏩ Skipping processing for Cloud Run update',
        {deploymentId}
      );
      return null;
    }
    ...
    try {
      ...
      if (!templateName) {
        console.log(
          '[FirestoreTrigger] ⚠️ No deployment template name found, skipping',
          {deploymentId}
        );
        return null;
      }
      ...
      console.log('[FirestoreTrigger] 🔑 Getting auth token...', {
        deploymentId,
      });
      ...
      if (!cloudRunUrl) {
        console.error(
          '[FirestoreTrigger] 🚫 Cloud Run URL is not configured. Please set CLOUD_RUN_URL environment variable',
          {deploymentId}
        );
      ...
      console.log(
        '[FirestoreTrigger] 🚀 Calling Cloud Run service:',
        cloudRunUrl,
        {deploymentId}
      );
      ...
      console.error(
        '[FirestoreTrigger] 🔥 Error processing deployment:',
        error,
        {deploymentId}
      );
      throw error;
...
exports.deploymentUpdated = onDocumentUpdated(
  'deployments/{deploymentId}',
  async event => {
    if (!event.data) {
      console.error('No data associated with the event');
    ...
    try {
      ...
      // sandbox deleted.
      if (
        afterData.status == 'delete_requested' &&
        beforeData.status != 'delete_requested'
      ) {
        // Get auth token for Cloud Run
        console.log('[FirestoreTrigger] 🔑 Getting auth token...', {
          deploymentId,
