...
export async function deployInfrastructure(
  ...
  console.log(
    `[InfraManager] 🚀 Starting deployment process for document ${documentId}`,
    {
      documentId,
      templateType,
      region,
  ...
  try {
    ...
    console.log(
      `[InfraManager] 🚀 Sending request to Infrastructure Manager API:`,
      {
        documentId,
        deploymentId,
        url: apiUrl,
    ...
    while (!deploymentComplete && retryCount < maxRetries) {
      ...
      try {
        ...
        console.log(`[InfraManager] 🔄 Checking operation status:`, {
          documentId,
          operationStatus: operationResponse.data.done
            ? 'complete'
            : 'in progress',
        ...
        console.warn(
          `[InfraManager] ⚠️ Error checking operation status (will retry):`,
          {
            documentId,
            error: error instanceof Error ? error.message : 'Unknown error',
            retry: retryCount + 1,
    ...
    if (!deploymentComplete) {
      console.log(
        `[InfraManager] ⏱️ Deployment status check timeout reached, continuing in background:`,
        {
          documentId,
        }
      );
      ...
      await docRef.update({
        status: 'provision_inprogress',
        message:
          'Deployment continuing in background. Check console for latest status.',
        updatedAt: new Date(),
        _updateSource: 'cloudrun',
        consoleUrl: `https://console.cloud.google.com/config-management/deployments/detail/${region}/${deploymentId}?project=${config.project.id}`,
      });
