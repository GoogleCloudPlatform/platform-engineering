...
def password_rotation_function(cloud_event: CloudEvent):
  ...
  reset_password_status = reset_password(instance_name,db_name,location,db_user,db_pass,new_db_pass)
  ...
  verify = verify_change_password(instance_name,db_name,location,db_user,new_db_pass)
  if verify:
...
def verify_change_password(instance_name,db_name,location,db_user,db_pass):
  ...
  try:
    with pool.connect() as db_conn:
      results = db_conn.execute(sqlalchemy.text("select * from information_schema.tables limit 1;")).fetchall()
      for row in results:
        print(row)
    return True
  except Exception as e:
