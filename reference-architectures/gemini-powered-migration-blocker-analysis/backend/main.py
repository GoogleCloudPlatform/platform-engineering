...
async def process_report_generation(
    ...
    if not GCP_PROJECT_ID or not GCP_LOCATION or not GCS_BUCKET_NAME:
        print(
            f"Error processing report {report_id}: Missing runtime GCP config (Project, Location, or Bucket Name)."
        )
        try:
            ...
            await update_report_status(
                report_id,
                status="failed",
                error="Missing/invalid server configuration for GCP.",
            )
        except Exception as update_e:
            print(
                f"Additionally, failed to update report status to 'failed': {update_e}"
            )
    ...
    try:
        ...
        if repo_info.get("error"):
            ...
            await update_report_status(
                report_id, status="failed", error=repo_info["error"]
            )
        ...
        if not PROMPT_TEMPLATE_PATH.is_file():
            ...
            await update_report_status(
                report_id,
                status="failed",
                error="Server configuration error: Prompt template missing.",
            )
            return  # Stop processing
        ...
        try:
            ...
            print(
                f"Error reading prompt template file {PROMPT_TEMPLATE_PATH}: {file_read_e}"
            )
            await update_report_status(
                report_id,
                status="failed",
                error="Server error: Could not read prompt template.",
            )
            return
        ...
        prompt_context = {
            ...
            "dependency_files_content": repo_info.get("dependencies")
            or "Dependency files not found or empty.",
            "dockerfile_content": repo_info.get("dockerfile")
            or "Dockerfile not found or empty.",
        }
        try:
            ...
            await update_report_status(
                report_id,
                status="failed",
                error=f"Server configuration error: Invalid prompt template (missing key {format_e}).",
            )
            return
            ...
            await update_report_status(
                report_id,
                status="failed",
                error="Server configuration error: Could not format prompt template.",
            )
            return
        ...
        # --- Call Gemini ---
        print(
            f"Calling Gemini model '{GEMINI_MODEL_NAME}' (expecting JSON output) for report {report_id}..."
        )
        analysis_result_str = (
            await analyze_with_gemini(  # Store raw string result first
        ...
        if analysis_result_str.startswith("Error:"):
            print(
                f"Gemini analysis failed for report {report_id}: {analysis_result_str}"
            )
            # Store the specific Gemini error message
            await update_report_status(
                report_id, status="failed", error=analysis_result_str
            )
            return  # Stop processing
        ...
        try:
            ...
            print(
                f"Successfully parsed Gemini response as JSON for report {report_id}."
            )
        except json.JSONDecodeError as json_err:
            print(
                f"Warning: Failed to parse Gemini response as JSON for report {report_id}. Error: {json_err}"
            )
            # Log the cleaned string that failed parsing for debugging
            print(
                f"Storing raw response string instead. Cleaned response was:\n{cleaned_analysis_str[:500]}..."
            )
            final_summary_data = (
                analysis_result_str  # Store the ORIGINAL raw string on failure
            )
        ...
        print(
            f"Successfully processed and saved report: {report_id} (JSON parsing {'succeeded' if isinstance(final_summary_data, dict) else 'failed'})."
        )

    except FileNotFoundError as fnf_err:
        # Catch specific error if update_report_status fails because doc deleted mid-process
        print(
            f"Error processing report {report_id}: Document not found during update. {fnf_err}"
        )
        ...
        try:
            ...
            print(
                f"Additionally, failed to update report {report_id} status to 'failed': {update_e}"
            )
...
async def create_report(
    ...
    documentation_files: List[UploadFile] = File(
        [], description="Upload documentation files (optional)"
    ),
):
    """
    ...
    for doc_file in documentation_files:
        if doc_file.filename:  # Check if a file was actually uploaded
            ...
            try:
                ...
                print(
                    f"An unexpected error occurred processing file {doc_file.filename}: {e}"
                )
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_report(report_id: str):
    ...
    if (
        ...
        and metadata.report_content_gcs_uri
    ):
        ...
        analysis_result = (
            f"Full report stored at: {metadata.report_content_gcs_uri}"  # Placeholder
        )
